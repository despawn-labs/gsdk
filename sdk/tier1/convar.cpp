...
bool CCommand::Tokenize(const char *pCommand, cmd_source_t source,
                        characterset_t *pBreakSet) {
  Reset();
  m_source = source;
...
void ConVar::InstallChangeCallback(FnChangeCallback_t callback,
  ...
  if (bInvoke) {
    callback(this, m_Value.m_pszString,
             ObscureConvarValue(m_Value.m_fValue, (intp)this));
  }
}
...
bool ConVar::InternalSetColorFromString(const char *value) {
  ...
  if (nParamsRead >= 3) {
    ...
    if (nRGBA[0] >= 0 && nRGBA[0] <= 255 && nRGBA[1] >= 0 && nRGBA[1] <= 255 &&
      ...
      m_Value.m_fValue =
          ObscureConvarValue((float)(m_Value.m_nValue), (intp)this);
    }
  }
...
void ConVar::InternalSetValue(const char *value) {
  ...
  if (!InternalSetColorFromString(value)) {
    ...
    double dblValue =
        V_atod(value); // Use double to avoid 24-bit restriction on integers and
                       // allow storing timestamps or dates in convars
    ...
    m_Value.m_nValue = ObscureConvarValue(
        (int)dblValue,
        (intp)this); // convert double to int to avoid precision loss
  }
...
void ConVar::Create(const char *pName, const char *pDefaultValue,
  ...
  if (!InternalSetColorFromString(m_Value.m_pszString)) {
    double dblValue = V_atod(
        m_Value
            .m_pszString); // Use double to avoid 24-bit restriction on integers
                           // and allow storing timestamps or dates in convars
