...
class CUtlFilenameSymbolTable {
  ...
  struct FileNameHandleInternal_t {
    ...
    // We pack the path and file values into a single 32 bit value.  We were
    // running out of space with the two 16 bit values (more than 64k files) so
    // instead of increasing the total size we split the underlying pool into
    // two (paths and files) and use a smaller path string pool and a larger
    // file string pool.
    unsigned int value;

#ifdef PLATFORM_64BITS
    // some padding to make sure we are the same size as FileNameHandle_t on 64
    // bit.
    unsigned int pad;
#endif
    ...
    unsigned int GetPath() const {
      return ((value >> cNumBitsInFile) & cPathBitMask);
    }
    void SetPath(unsigned int path) {
      Assert(path < cMaxPathValue);
      value =
          ((value & cFileBitMask) | ((path & cPathBitMask) << cNumBitsInFile));
    }

    // Part after the final '/', including extension
    unsigned int GetFile() const { return (value & cFileBitMask); }
    void SetFile(unsigned int file) {
      Assert(file < cMaxFileValue);
      value =
          ((value & (cPathBitMask << cNumBitsInFile)) | (file & cFileBitMask));
    }
  };
...
public:
  ...
  int PathIndex(const FileNameHandle_t &handle) {
    return ((const FileNameHandleInternal_t *)&handle)->GetPath();
  }
